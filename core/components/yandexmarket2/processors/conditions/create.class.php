...
class ymConditionCreateProcessor extends modObjectCreateProcessor
{
    ...
    public function beforeSet()
    {
        ...
        if (empty($column)) {
            $this->modx->error->addField('column', $this->modx->lexicon('ym_condition_column_err_ns'));
        } elseif (empty($operator) || !array_key_exists($operator, Condition::OPERATOR_SYMBOLS)) {
            $this->modx->error->addField('operator', $this->modx->lexicon('ym_condition_operator_err_ns'));
        }
...
return ymConditionCreateProcessor::class;
